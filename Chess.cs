...
namespace Chess {
    ...
    // The delegate type used to find out what Rank to promote pawns to
    public delegate Rank PawnCallback();

	public class ChessGame {
        #region Variables

        // When TESTING is true, pieces of any color can be moved, regardless of the Turn. Makes testing much easier
        private static readonly bool TESTING = false;

        // The 8x8 board containing the chess pieces (or null if no piece is on the square)
		...
		ChessPiece this [Point pos] {
			get {return board[pos.X, pos.Y];}
			set {board[pos.X, pos.Y] = value;}
		}
	
        // Contains information about the status of the move just attempted
        public string Status { get; internal set; }

        // BLACK or WHITE
        public Color Turn { get; internal set; }

        // True once the game is over (no more moves allowed)
        public bool GameOver { get; internal set; }

        // A stack of the moves made since the beginning of the game
        ...
        // Used for castle status
        internal static readonly int LEFT = 0, RIGHT = 1;
        #endregion
        ...
        // Sets up a new Chess game. Wipes any status from previous game
        public void newGame() {
            Turn = WHITE;
        ...
        public static bool IsValidSquare(Point square) {
        ...
        ChessMove AttemptMove(Point from, Point to) {
            ChessPiece piece = this[from];
        ...
        bool IsPieceBetween(Point from, Point to) {
            int dX = Math.Abs(from.X - to.X);
            int dY = Math.Abs(from.Y - to.Y);
        ...
        ChessMove AttemptMovePawn(ChessMove move) {
        ...
        ChessMove AttemptEnPassant(ChessMove move) {
            if (moveStack.Count == 0)
                return null;
        ...
        bool IsCastleAllowed(Point from, Point to) {
            // Check castle flag
        ...
        bool IsCheckAfterMove(Point from, Point to, Point kingSquare) {
            ChessMove move = AttemptMove(from, to);
        ...
        void UpdateCheckAndMateStatus(ChessMove move) {
        ...
        bool IsAvaliableMove(Color color) {
            Point kingSquare = FindKing(color);
        ...
        /* Return a list of possible moves for the piece at <square>
        ...
        */ 
        List<Point> GetPossibleMoves(Point square) {
        ...
        void ProcessCastleStatus(ChessMove move) {
